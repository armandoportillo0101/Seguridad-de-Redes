...
## Solución
```
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$ wget https://mercury.picoctf.net/static/a6d9cac3bfa4935ceb50c145d3ff5586/keygenme-trial.py
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$ cat keygenme-trial.py
#============================================================================#
#============================ARCANE CALCULATOR===============================#
#============================================================================#
...
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$ nano
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$ nano solve.py
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$ python3 solve.py
picoCTF{1n_7h3_|<3y_of_54ef6292}
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$ cat solve.py
#!/usr/bin/env python3
...
print(key_prefix)
porti_04@DESKTOP-8HT902T:~/pico_retos/tercer_parcial/keygenme$
...
## Notas Adicionales
Este juego está lejos de estar completo y solo tiene 243 líneas de Python. Al leer el código fuente, queda claro que la `check_key`función es la clave para obtener la bandera. Este código toma valores posicionales del resumen hexadecimal SHA256 del nombre de usuario al que se le otorgó la licencia de este software.

En la parte superior del código fuente, se encuentra la plantilla esperada para la clave, que está en `picoCTF{}`formato de bandera. Escribir un generador de claves para este software fue sencillo:

|   |   |
|---|---|
|1<br><br>2<br><br>3<br><br>4<br><br>5<br><br>6<br><br>7<br><br>8<br><br>9<br><br>10<br><br>11<br><br>12<br><br>13<br><br>14<br><br>15<br><br>16<br><br>17<br><br>18<br><br>19<br><br>20|`#!/usr/bin/env python3`<br><br>`import` `hashlib`<br><br>`username` `=` `b``"YOUR_USERNAME_HERE"`<br><br>`key_prefix` `=` `"picoCTF{1n_7h3_\|<3y_of_"`<br><br>`user_hash` `=` `hashlib.sha256(username).hexdigest()`<br><br>`key_prefix` `+``=` `user_hash[``4``]`<br><br>`key_prefix` `+``=` `user_hash[``5``]`<br><br>`key_prefix` `+``=` `user_hash[``3``]`<br><br>`key_prefix` `+``=` `user_hash[``6``]`<br><br>`key_prefix` `+``=` `user_hash[``2``]`<br><br>`key_prefix` `+``=` `user_hash[``7``]`<br><br>`key_prefix` `+``=` `user_hash[``1``]`<br><br>`key_prefix` `+``=` `user_hash[``8``]`<br><br>`key_prefix` `+``=` `"}"`<br><br>`print``(key_prefix)`|
